<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','发货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="dingdanfahuoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发货':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','发货') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="dingdanfahuoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发货':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','发货') && contents.tableBtnIcon == 0" type="success" size="mini" @click="dingdanfahuoCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'发货':'' }}</el-button>
    ...
    <dingdanfahuo-cross-add-or-update v-if="dingdanfahuoCrossAddOrUpdateFlag" :parent="this" ref="dingdanfahuoCrossaddOrUpdate"></dingdanfahuo-cross-add-or-update>
...
import dingdanfahuoCrossAddOrUpdate from "../dingdanfahuo/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      dingdanfahuoCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    dingdanfahuoCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.dingdanfahuoCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
      this.$refs.dingdanfahuoCrossaddOrUpdate.init(row.id,type);
      });
    },
    payHandler(row){
